...
当出现参数错误、工具调用错误等问题时，尝试自行修改输入参数、重新检查并选择工具以排除错误。
...
if __name__ == '__main__':
    ...
    calculate_desc="一个简单的计算器，当你需要进行基础的代数计算时，应使用此工具。"
    tool_executor.registerTool("Calculate", calculate_desc, calculate)
    tool_executor.registerTool("Search", search_desc, search)
    agent = ReActAgent(llm_client=llm, tool_executor=tool_executor)
    #question = "一千九百八十二加一除以九十六减去四十二加上括号内七十二除以九等于几"
    question = "直接使用Calculate工具查找小米手机最新型号"
    agent.run(question)